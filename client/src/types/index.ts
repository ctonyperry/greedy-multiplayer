...
export interface GameState {
  ...
  /** Entry threshold for getting on the board (from game settings) */
  entryThreshold: number;
  /** Target score to trigger final round (from game settings) */
  targetScore: number;
}
...
export interface UserPreferences {
  ...
  turnTimerPreference: number; // seconds, 0 = no auto-timeout
  aiTakeoverStrategy?: string; // AI plays if timer expires
}
...
/** Multiplayer player (extends PlayerState) */
...
export interface GameSettings {
  targetScore: number;
  entryThreshold: number;
  maxTurnTimer: number; // Game-level max (host sets)
}
